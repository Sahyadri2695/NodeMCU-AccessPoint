...
graph TD;
A(Start)
A --> B{Check if SSID, PW & static IP with DnS<br/> are present in the EEPROM}
B --> |Yes|G[Start station mode]
B --> |No|C[Start AP mode]
C --> D[Get SSID, PW, IP and Dns from the user]
D --> E[Store the above data in EEPROM]
E --> F[Reboot]
F --> B
G --> Z(Stop)
```
